...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(viewType && viewFilter) {
        viewType.addEventListener('change', (e) => {
            ...
            // 🌟 FIX: Auto-render grid when switching between By Class / By Teacher
            if (document.getElementById('opMode').value === 'regular') window.generateGrid(true);
        });

        // 🌟 FIX: Auto-render grid immediately when dropdown filter value is changed
...
window.generateGrid = function(isEngineCall = false) {
    ...
    // Only rebuild completely if PROCESS was manually clicked
...
function updateClassLoadUI() {
    ...
    // Also include classes that were manually added!
